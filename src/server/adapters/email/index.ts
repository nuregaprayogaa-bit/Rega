...
export async function sendEmail(msg: EmailMessage): Promise<void> {
  try {
    ...
    const from = process.env.EMAIL_FROM || "Rega <onboarding@resend.dev>";
...
/** Bungkus konten dalam template HTML sederhana ber-merek Rega. */
export function emailTemplate(opts: {
  ...
  const cta =
    opts.ctaLabel && opts.ctaUrl
      ? `<a href="${appUrl}${opts.ctaUrl}" style="display:inline-block;background:#4f46e5;color:#fff;text-decoration:none;padding:10px 18px;border-radius:8px;font-weight:600;margin-top:16px">${opts.ctaLabel}</a>`
      : "";
  return `
  <div style="font-family:system-ui,Segoe UI,Arial,sans-serif;max-width:480px;margin:0 auto;padding:24px;color:#0f172a">
    <p style="font-size:20px;font-weight:700;color:#4f46e5;margin:0 0 16px">Rega</p>
    ...
    <p style="font-size:12px;color:#94a3b8;margin:0">Email ini dikirim otomatis oleh Rega — marketplace jasa freelance Indonesia.</p>
  </div>`;
}
